...
public class ElementGunFireRightclickedProcedure {
		...
		if (InputConstants.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), InputConstants.KEY_LSHIFT)) {
			{
				...
				if (_plr.level().isClientSide())
					return;
				net.minecraft.world.item.Item fromItem = net.minecraftforge.registries.ForgeRegistries.ITEMS.getValue(new net.minecraft.resources.ResourceLocation("enteringyourmind:element_gun_fire"));
				net.minecraft.world.item.Item toItem = net.minecraftforge.registries.ForgeRegistries.ITEMS.getValue(new net.minecraft.resources.ResourceLocation("enteringyourmind:element_gun_water"));
			...
			if (itemstack.getItem() instanceof ElementGunFireItem)
				itemstack.getOrCreateTag().putString("geckoAnim", "animation.elementgunfire.shoot");
			for (int index0 = 0; index0 < 10; index0++) {
				particleAmount = 15;
				particleRadius = 1.5;
				for (int index1 = 0; index1 < (int) particleAmount; index1++) {
					world.addParticle((SimpleParticleType) (EnteringyourmindModParticleTypes.FIRE.get()),
							(entity.level().clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(3)), ClipContext.Block.OUTLINE, ClipContext.Fluid.NONE, entity)).getBlockPos().getX() + 0
									+ Mth.nextDouble(RandomSource.create(), -1, 1) * particleRadius),
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = EnteringyourmindModEntities.INVIS_FIRE.get().spawn(_level, BlockPos.containing(x + 3, y, z + 3), MobSpawnType.MOB_SUMMONED);
